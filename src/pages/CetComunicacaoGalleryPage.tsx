...
import { cetComunicacaoSections } from "../data/cetComunicacaoGallery";
import { useI18n } from "../i18n/useI18n";

export function CetComunicacaoGalleryPage() {
  const { copy } = useI18n();
  const page = copy.cetComunicacaoGalleryPage;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a0612]">
      ...
      <main className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16 lg:max-w-7xl">
        <div className="max-w-3xl">
          ...
          <a
            href="https://github.com/victordanbc/cetcomunicacao"
            target="_blank"
            rel="noreferrer"
        ...
        <div className="mt-14 space-y-16">
          {cetComunicacaoSections.map((section, sectionIndex) => {
            const slideOffset = cetComunicacaoSections
              ...
              <section key={section.sectionKey}>
                ...
                <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 lg:gap-5">
                  {section.slides.map((slide, slideIndex) => (
                    <figure
                      key={slide.src}
                      className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-[#120e1c]"
                    >
                      <figcaption className="border-b border-slate-200 px-2 py-2 text-xs font-medium text-slate-700 dark:border-white/10 dark:text-zinc-300 sm:px-3 sm:py-2.5 sm:text-sm">
                        {copy.projects.cetComunicacaoGallery[slide.labelKey]}
                      </figcaption>
                      <LazyScreenshot
                        src={slide.src}
                        alt={copy.projects.cetComunicacaoGallery[slide.labelKey]}
