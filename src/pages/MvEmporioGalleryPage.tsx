...
import { mvEmporioSections } from "../data/mvEmporioGallery";
import { useI18n } from "../i18n/useI18n";

export function MvEmporioGalleryPage() {
  const { copy } = useI18n();
  const page = copy.mvEmporioGalleryPage;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a0612]">
      ...
      <main className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16 lg:max-w-7xl">
        <div className="max-w-3xl">
          ...
          <a
            href="https://github.com/vitorpvpcascavel-maker/mvemporio"
            target="_blank"
            rel="noreferrer"
        ...
        <div className="mt-14 space-y-16">
          {mvEmporioSections.map((section, sectionIndex) => {
            const slideOffset = mvEmporioSections
                  ...
                  {section.slides.map((slide, slideIndex) => (
                    ...
                    >
                      <figcaption className="border-b border-slate-200 px-2 py-2 text-xs font-medium text-slate-700 dark:border-white/10 dark:text-zinc-300 sm:px-3 sm:py-2.5 sm:text-sm">
                        {copy.projects.mvEmporioGallery[slide.labelKey]}
                      </figcaption>
                      <LazyScreenshot
                        src={slide.src}
                        alt={copy.projects.mvEmporioGallery[slide.labelKey]}
